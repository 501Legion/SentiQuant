...
## 11. Stage 1 결과 — 매트릭스 실행 (2026-06-13)

`scripts/bt_matrix_momentum.py`, 5/13~6/11, finbert-wsb/sentiment/opinion_trend/community_liquid. (OHLCV 캐시 hot → throttle 0)

| arm | net% | 거래 | 승률 | MDD | 최대승자 | 최대패자 |
|---|---|---|---|---|---|---|
| TIGHT × LLM-OFF | +0.67 | 3 | 67% | -0.3 | +13.1 | -5.5 |
| TIGHT × LLM-ON (≈운영) | +0.67 | 3 | 67% | -0.3 | +13.1 | -5.5 |
| LOOSE × LLM-OFF (순수 모멘텀) | -0.08 | 9 | 33% | -1.2 | **+20.6** | -12.2 |
| LOOSE × LLM-ON (브레이크-온-모멘텀) | +0.29 | 6 | 33% | -0.4 | +13.1 | -12.2 |

**계측 버그 정정**: 초판은 `_trades()`가 거래 객체에 없는 `net_return_pct`를 읽어 per-trade 수익률이 전부 +0.0%로 나옴 → 실제 필드 `pnl_pct`로 수정. win_rate도 `:.0%`(이미 퍼센트인데 ×100=6670%)를 `:.0f%`로 수정. **집계치(net/거래/MDD/router_dist)는 처음부터 유효, 분포(top/bottom/SNDK)만 깨졌었음.**

**판정 — 헤드라인은 보수 우세, 그러나 메커니즘은 가설대로**:
1. 이 윈도에선 TIGHT가 이김(+0.67% vs LOOSE×OFF -0.08%). 완화는 거래 3→9, 승률 67→33%, MDD 4배 악화. 평균만 보면 "완화는 손해".
2. **그러나 LOOSE×OFF만 매트릭스 최대 승자 +20.6%를 잡음**(TIGHT는 +13.1 천장). 완화가 더 두꺼운 우측꼬리를 실제로 끌어냄 = 모멘텀 가설 그대로. 짧은 윈도라 딸려온 패자(-12.2/-9.8/-6.5)가 그 꼬리를 압도했을 뿐.
3. **핵심: LLM 브레이크는 평균을 "고치되" 최고 승자를 참수해서 고친다.** LOOSE×ON이 net -0.08→+0.29, MDD -1.2→-0.4로 개선했지만 그 대가가 **+20.6%→+13.1% 승자 절단**. 패자도 깎지만 최고 승자도 같이. → 큰 승자 없는 윈도엔 이득, SNDK급 사건 있는 윈도엔 치명적이라는 가설의 정확한 실증.
4. **TIGHT에선 LLM 효과 realized 0**: OFF/ON 둘 다 net +0.67%·동일 분포. LLM이 HOLD를 12건 추가했지만 닫힌 거래는 동일 3건 → "LLM 의사판단만"(§6) 재확인.

**결정적 한계**: **전 arm SNDK 미포착**. 윈도가 6/11에 끝나 SNDK 신호일(6/13)보다 먼저 끝나기 때문 — 검증하려던 우측꼬리 사건이 윈도 밖. 즉 헤드라인 P&L은 "큰 사건 없는 윈도"의 예측된 null이고, 모멘텀 가설을 반증 못 함. +20.6% 케이스가 메커니즘(완화→꼬리↑, 브레이크→꼬리 절단)을 대신 입증.

**다음 단계(결정적 실험)**: 윈도를 **6/13 포함**으로 확장해 SNDK 사건을 안에 넣고 동일 매트릭스 재실행. 캐시 키가 `~06-11`이라 06-13까지 델타만 prefetch 필요(`prefetch_ohlcv_window.py`로 윈도 키 변경). 그 윈도에서 LOOSE×OFF가 SNDK를 잡고 LOOSE×ON이 그걸 절단하는지가 Stage 2 착수 여부를 가른다.
