# Archive Index — 2026-04

| Feature | 완료일 | Match Rate | 핵심 내용 |
|---------|--------|-----------|---------|
| [news-rsi-trading](news-rsi-trading/) | 2026-03 | — | 기반 시스템: RSI + TextBlob 신호 생성 |
| [polygon-massive-migration](polygon-massive-migration/) | 2026-04-01 | 98% | Polygon SDK 마이그레이션 |
| [market-filter-finbert](market-filter-finbert/) | 2026-04-02 | 100% | QQQ Market RSI Filter + FinBERT 병렬 감성 |
| [signal-v2](signal-v2/) | 2026-04-05 | 98% | FinBERT neutral 필터 + Volume Spike + 백테스팅 인프라 |
| [reddit-gpt4-quant](reddit-gpt4-quant/) | 2026-04-17 | 100% | Reddit 파이프라인 + GPT-4 + 12전략 비교 |
| [daily-thread-collector](daily-thread-collector/) | 2026-04-18 | 100% | Daily Thread 댓글 수집 (99% 커버리지 복구) |
| [wsb-finbert-preprocessor](wsb-finbert-preprocessor/) | 2026-04-18 | 100% | WSB 전처리 101개 슬랭 + finbert-wsb 옵션 |
| [wsb-signal-v3](wsb-signal-v3/) | 2026-04-22 | — | 30MA 제거 + Velocity 보정 매트릭스 + 5단계 청산 |
